...
Function NI1A_CreateHistogram(orientation)
	...
	Duplicate/O Qvector, $("root:Packages:Convert2Dto1D:TwoTheta_"+orientation), $("root:Packages:Convert2Dto1D:Dspacing_"+orientation)
	Duplicate/O Qvector, $("root:Packages:Convert2Dto1D:TwoThetaWidth_"+orientation), $("root:Packages:Convert2Dto1D:DspacingWidth_"+orientation)
...
Function NI1A_Create2DQWave(DataWave)
	...
	if(abs(HorizontalTilt)>0.01 && abs(VerticalTilt)>0.01)
		Q2DWave = sqrt( ( NI1T_TiltedToCorrectedR( (p-BeamCenterX)*PixelSizeX ,SampleToCCDDistance,HorizontalTilt) )^2 + (NI1T_TiltedToCorrectedR((q-BeamCenterY)*PixelSizeY,SampleToCCDDistance,VerticalTilt))^2)
		print "Both tilts used, time was = "+num2str((ticks-ts)/60)
	elseif(abs(HorizontalTilt)>0.01 && abs(VerticalTilt)<=0.01)
		Q2DWave = sqrt( ( NI1T_TiltedToCorrectedR( (p-BeamCenterX)*PixelSizeX ,SampleToCCDDistance,HorizontalTilt) )^2 + ((q-BeamCenterY)*PixelSizeY)^2)
		print "Horizontal tilt used, time was = "+num2str((ticks-ts)/60)
	elseif(abs(HorizontalTilt)<=0.01 && abs(VerticalTilt)>0.01)
		Q2DWave = sqrt( ((p-BeamCenterX)*PixelSizeX)^2 + (NI1T_TiltedToCorrectedR((q-BeamCenterY)*PixelSizeY,SampleToCCDDistance,VerticalTilt))^2)
		print "Vertical tilt used, time was = "+num2str((ticks-ts)/60)
	else
		Q2DWave = sqrt(((p-BeamCenterX)*PixelSizeX)^2 + ((q-BeamCenterY)*PixelSizeY)^2)
		print "No tilts used, time was = "+num2str((ticks-ts)/60)
	endif
	Q2DWave[beamCenterX][beamCenterY] = NaN
	//the QsDWave now contains the distance from beam center, corrected for any tilt if necessary....  Results should be in mm.... 
	//added to calculate the theta values...
	MatrixOP/O Theta2DWave=Q2DWave
	Theta2DWave = atan(Q2Dwave/SampleToCCDDistance)/2
...
//first data selection part
	...
	PopupMenu SelectEndOfRange,pos={158,245},size={211,21},proc=NI1A_PopMenuProc,title="End"
